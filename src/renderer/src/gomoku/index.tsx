...
const GomokuGame = () => {
    ...
    const [blackTotalTime, setBlackTotalTime] = useState(TOTAL_TIME_SECONDS);
    const [whiteTotalTime, setWhiteTotalTime] = useState(TOTAL_TIME_SECONDS);
    const [moveTimeLeft, setMoveTimeLeft] = useState(MOVE_TIME_SECONDS);

    // 当前棋盘状态
    const currentBoard = history[currentMove];
    // 当前执子方 (黑先白后)
    const isBlackNext = currentMove % 2 === 0;
    const ai = useMemo(() => createAi(BOARD_SIZE, {
        defendWeight: 0.95,
        centerWeight: 1,
        weights: {
            fourOpen: 12000,
    ...
    useEffect(() => {
        ...
        const move = ai.findBestMove(currentBoard, checkWinner);
        if (!move) return;
        const timer = setTimeout(() => {
            handleSquareClick(move.r, move.c);
        }, ai.config.thinkTimeMs);
    ...
    useEffect(() => {
        ...
        const tick = setInterval(() => {
            setMoveTimeLeft((prev) => {
                if (prev <= 1) {
                    setWinner(isBlackNext ? 'white' : 'black');
                    return 0;
                }
                return prev - 1;
            });
    ...
    }, [currentMove, winner, isBlackNext]);

    useEffect(() => {
        setMoveTimeLeft(MOVE_TIME_SECONDS);
    }, [currentMove]);
    ...
    const resetGame = () => {
        ...
        setWinner(null);
        setWinningLine([]);
        setBlackTotalTime(TOTAL_TIME_SECONDS);
        setWhiteTotalTime(TOTAL_TIME_SECONDS);
        setMoveTimeLeft(MOVE_TIME_SECONDS);
    };
        ...
        <div className="relative flex h-full w-full min-h-0 flex-col items-center justify-start bg-stone-100 p-4 font-sans text-stone-800 overflow-hidden">
            ...
            <div className="absolute right-4 top-4 text-right text-xs font-bold text-stone-700">
                <div className="flex flex-col gap-1">
                    <span>黑方总时: {formatTime(blackTotalTime)}</span>
                    <span>白方总时: {formatTime(whiteTotalTime)}</span>
                    <span className={moveTimeLeft <= 10 ? 'text-red-500' : ''}>本步剩余: {formatTime(moveTimeLeft)}</span>
                </div>
            </div>
